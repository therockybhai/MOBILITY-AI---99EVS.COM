...
// Converts completed MobilityAI diagnostics into structured public reports
// ═══════════════════════════════════════════════════════════════════════════

// ── REPORT SYSTEM PROMPT ──────────────────────────────────────────────────
const REPORT_SYSTEM = `You are the MobilityAI Public Diagnostic Report Generator for 99EVS.

Your task is to convert a completed vehicle diagnostic into a structured, standalone knowledge report that:
- Can be published as a public knowledge page
- Helps other users experiencing the same vehicle problem
- Remains neutral, educational, and safety-focused
- Never claims absolute mechanical diagnosis

REPORT GENERATION RULES
1. Extract the core vehicle problem from the diagnostic conversation
2. Write for a general audience — not just the original user
3. Use clear headings, structured sections, and accessible language
4. Always include the safety disclaimer
5. Always include related search terms in Section 7
6. Write Section 6 (Educational Insight) as a mini-explainer — teach the reader how the system works
7. Be specific with cause descriptions — not vague labels

MANDATORY DISCLAIMER (include verbatim at the bottom of every report):
⚠️ Disclaimer: This report provides educational diagnostic insights based on described symptoms. It does not constitute a final mechanical diagnosis and does not replace professional inspection. For safety-critical systems always consult a certified and qualified technician.

OUTPUT FORMAT — produce exactly this structure with no deviations:

---REPORT START---

## [Problem Title]
*Short, searchable, descriptive title of the vehicle issue*

---

### 1️⃣ Problem Summary
[2–3 sentences. Clear explanation of the problem. Vehicle type, system involved, symptom description. Written for any reader, not just the original user.]
...
**🔴 High Probability**
- [Cause] — [Engineering reason in plain language]
- [Cause] — [Engineering reason]
...
[2–3 sentences on safety implications. If HIGH or CRITICAL: explicit instruction not to drive until inspected.]
...
2. [Second step]
3. [Further steps]
4. [When to seek professional help — always include this]

*Estimated inspection cost: [₹ range if India-applicable]*

---

### 5️⃣ Vehicle Systems Involved
**Primary System:** [System name]
**Subsystem:** [Subsystem name]
**Knowledge Graph Path:** [Vehicle Type] → [System] → [Subsystem] → [Component]

---

### 6️⃣ Educational Insight — How This System Works
[3–5 sentences. Explain the vehicle system in plain language. Cover: what the system does, how it works in normal operation, how the failure described causes the symptom. No jargon without explanation. Written as a mini-explainer for someone who knows nothing about cars.]

---

### 7️⃣ Related Problems
Users who searched this problem also looked for:
- [Related problem 1]
- [Related problem 2]
- [Related problem 3]
- [Related problem 4]
...
[One clear paragraph on the specific conditions under which professional inspection is essential for this problem type. Make this specific to the system and risk level — not generic.]

---

*⚠️ Disclaimer: This report provides educational diagnostic insights based on described symptoms. It does not constitute a final mechanical diagnosis and does not replace professional inspection. For safety-critical systems always consult a certified and qualified technician.*

*Report generated by MobilityAI · 99EVS.com · Free Mobility Knowledge · No Ads · No Bias*

---REPORT END---`;

// ── IN-MEMORY REPORT STORE ────────────────────────────────────────────────
// Holds the last 50 generated reports (Phase 2: persist to database)
const REPORT_STORE = {
  reports: [],    // array of { id, title, slug, content, ts, vehicleType, system }
  maxSize: 50,

  add(report) {
    this.reports.unshift(report);
    if (this.reports.length > this.maxSize) this.reports = this.reports.slice(0, this.maxSize);
    return report;
  },
  ...
  getBySlug(slug) {
    return this.reports.find(r => r.slug === slug) || null;
  },
...
// Extracts structured metadata from generated report content
function extractReportMetadata(content) {
  const titleMatch  = content.match(/^##\s+(.+)$/m);
  const systemMatch = content.match(/\*\*Primary System:\*\*\s*(.+)/);
  const riskMatch   = content.match(/\*\*Risk Level:\s*([A-Z]+)\*\*/);
  const vehicleMatch = content.match(/Knowledge Graph Path:\*\*\s*\[?([^\]\n]+)/);

  return {
    title:       titleMatch   ? titleMatch[1].trim()   : 'Vehicle Diagnostic Report',
    system:      systemMatch  ? systemMatch[1].trim()  : 'Unknown System',
    riskLevel:   riskMatch    ? riskMatch[1].trim()    : 'UNKNOWN',
    vehicleType: vehicleMatch ? vehicleMatch[1].trim() : 'Unknown Vehicle'
  };
}
...
export default async function handler(req, res) {
  ...
  if (isKiai === true) {
    ...
    // Report store commands
    if (/report.*list|list.*report|recent.*report/i.test(kiaiMsg)) {
      const reports = REPORT_STORE.list(10);
      if (!reports.length) return res.status(200).json({ reply: 'No diagnostic reports generated yet.' });
    ...
    if (/report.*stats|report.*count/i.test(kiaiMsg)) {
      ...
      const byRisk = r.reduce((acc, x) => { acc[x.riskLevel] = (acc[x.riskLevel]||0)+1; return acc; }, {});
      const bySys  = r.reduce((acc, x) => { acc[x.system]    = (acc[x.system]||0)+1;    return acc; }, {});
      return res.status(200).json({ reply:
        \`**Report Store Stats**\n\nTotal reports: ${r.length}\n\nBy Risk Level:\n\${Object.entries(byRisk).map(([k,v])=>\`- \${k}: \${v}\`).join('\n')}\n\nBy System:\n\${Object.entries(bySys).map(([k,v])=>\`- \${k}: \${v}\`).join('\n')}\`
      });
    }
...
Adapt depth to the user's apparent knowledge level.\`,
    ...
    // REPORT MODE — Public Diagnostic Report Generator
    report: `

ACTIVE MODE: PUBLIC DIAGNOSTIC REPORT GENERATOR
You are generating a structured public knowledge report from a completed MobilityAI diagnostic.

Use the REPORT_SYSTEM format exactly. Produce the complete 8-section report.
Include all markers: ---REPORT START--- and ---REPORT END---
Write every section. Do not abbreviate or skip any section.
The report must function as a standalone public knowledge page.\`
  };

  // ── REPORT MODE: use dedicated REPORT_SYSTEM prompt ────────────────────
  if (mode === 'report') {
    // Report generation uses its own specialized system prompt
    // combined with the REPORT_SYSTEM constant defined above
    finalSystem = REPORT_SYSTEM + '\n\n' + (MODE_PROMPTS.report || '');
      ...
      if (valid.length > 0 && valid[0].role === 'user') {
        ...
        if (apiRes.ok) {
          ...
          if (reply && reply.length > 20) {
            ...
            if (mode === 'report' && reply.includes('---REPORT START---')) {
              const reportContent = reply
                .replace('---REPORT START---', '')
                .replace('---REPORT END---', '')
                .trim();
              const meta     = extractReportMetadata(reportContent);
              const reportId = generateReportId();
              const slug     = generateSlug(meta.title);
              REPORT_STORE.add({
                id: reportId, slug, title: meta.title,
                system: meta.system, riskLevel: meta.riskLevel,
                vehicleType: meta.vehicleType, content: reportContent,
                ts: new Date().toISOString()
              });
              ...
              return res.status(200).json({
                ...
                reportMeta: { title: meta.title, system: meta.system, riskLevel: meta.riskLevel, vehicleType: meta.vehicleType }
              });
            }
            return res.status(200).json({ reply });
          }
